...
public class T0004_PackReader extends TestCase {
    ...
    public void test001_scanPack() throws IOException {
        ...
        pr = new PackReader(null, new FileInputStream(TEST_PACK));
        itr = pr.iterator();
        while (itr.hasNext()) {
    ...
    public void test003_lookupCompressedObject() throws IOException {
        final PackReader pr;
        final ObjectId id;
        final PackedObjectReader r;

        id = new ObjectId("902d5476fa249b7abc9d84c611577a81381f0327");
        pr = new PackReader(null, TEST_PACK);
        r = pr.get(id);
        assertNotNull(r);
        assertEquals(id, r.getId());
        assertEquals(Constants.TYPE_TREE, r.getType());
        assertEquals(35, r.getSize());
        assertEquals(7738, r.getDataOffset());
        assertNull(r.getDeltaBaseId());
        r.close();
        pr.close();
    }
    ...
    public void test004_lookupDeltifiedObject() throws IOException {
        ...
        r = pr.get(id);
        assertNotNull(r);
        assertEquals(base, r.getDeltaBaseId());
        assertEquals(id, r.getId());
        assertEquals(Constants.TYPE_BLOB, r.getType());
        assertEquals(16512, r.getSize());
        assertEquals(537, r.getDataOffset());
        r.close();
        pr.close();
    }
}
